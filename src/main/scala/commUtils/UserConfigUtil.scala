...
object UserConfigUtil {
  ...
  def apply(propertiesName:String) = {
    val configurationUtil = new UserConfigUtil()
  ...
  //    val config: FileBasedConfiguration = ConfigurationUtil.apply("config.properties").config
  //    print(config.getString("jdbc.user"))
  //  }
}
